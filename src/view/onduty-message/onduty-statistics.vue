...
<script>
import { getMonthOndutyMsgStatistics, getYearOndutyMsgStatistics } from '@/api/onduty-message'
...
export default {
  ...
  methods: {
    getCurrentMonthStatistics () {
      getMonthOndutyMsgStatistics(formatDate(this.currentMonth)).then(res => {
        const data = res.data
    ...
    getCurrentYearStatistics () {
      getYearOndutyMsgStatistics(formatDate(this.currentYear)).then(res => {
        const data = res.data
