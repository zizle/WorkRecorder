  ...
  <Row>
    <List :header="dataShowStatus" border size="small" item-layout="vertical">
      <ListItem v-for="item in messageList" :key="item.id">
        <ListItemMeta :title="item.create_time" />
        <Row>
...
import { getOndutyMessage, delOndutyMessageRecord } from '@/api/onduty-message'
import { formatDate } from '@/libs/util'
export default {
  name: 'ondutyMessageMy',
  data () {
    return {
      startDate: new Date(),
      endDate: new Date(),
  ...
  methods: {
    ...
    getCurrentMessage () {
      ...
      getOndutyMessage(reqData).then(res => {
        const data = res.data
