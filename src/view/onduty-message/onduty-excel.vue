      ...
      <Row>
        <Upload
          ...
          <div style="padding: 20px 0">
              ...
              <p>点击或拖动文件到这里上传值班信息</p>
          </div>
        </Upload>
      </Row>
      <Row>
        <div>文件格式：后缀【xlsx】或【xls】的EXCEL合法文件。</div>
        <div>表格格式：Sheet名为【值班信息】字样，第一行为表头且各列为【日期，信息内容，备注】字样,其他列确保无内容。</div>
        <div style="color:#fb700d">特别注意：1. 第一列【日期】为日期格式。2. 要添加系统已存在的记录日期之前的数据需使用【手动添加】。</div>
        <div>错误排查：系统只会读取比当前已存在的数据日期大的数据行。例如：当前系统<span style="color:#fb700d">记录截止2020.12.31</span>，上传<span style="color:#fb700d">只读2021.01.01及之后日期</span>的记录。</div>
      </Row>
    </Card>
    <br>
...
<script>
import { ondutyUploadExcel, delOndutyMessageRecord, delOndutyMsgWithJoinTime } from '@/api/onduty-message'
...
export default {
  ...
  methods: {
    handleUploadExcel (file) {
      ...
      ondutyUploadExcel(formData).then(res => {
        const data = res.data
        // 将返回的数据展示到表格中
        this.$Modal.success(
          {
      ...
      if (this.uploadJoinTime > 0) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            delOndutyMsgWithJoinTime(reqData).then(res => {
              const data = res.data
              this.newMsgList = []
        ...
        this.$Message.error('您还没上传值班信息数据!')
      }
    }
  },
  created () {
